...
resource "aws_iam_policy" "invoke_lambda_policy-${prog_name}" {
  name        = "invoke_lambda_policy-${prog_name}"
  description = "Invoke another Lambda"
  policy = templatefile("./policy_invoke_lambda.json",{})
}
resource "aws_iam_role_policy_attachment" "invoke_lambda_policy-${prog_name}" {
    role        = aws_iam_role.${prog_name}-role.name
    policy_arn  = aws_iam_policy.invoke_lambda_policy-${prog_name}.arn
}
